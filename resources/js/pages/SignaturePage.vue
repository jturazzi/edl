...
<div v-else class="max-w-2xl mx-auto">
    ...
    <div class="bg-white rounded-2xl border border-gray-200 shadow-sm p-4 sm:p-6 mb-6">
        <h1 class="text-xl sm:text-2xl font-bold text-gray-900 mb-4 sm:mb-5">✍️ Signature du document</h1>
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-4 text-sm sm:text-base">
            <div class="text-gray-500 font-medium">Adresse</div>
            <div class="font-semibold text-gray-800">{{ edl.adresse }}, {{ edl.ville }}</div>
            ...
            <template v-if="edl.locataire_full_name">
                <div class="text-gray-500 font-medium">Locataire</div>
                <div class="font-semibold text-gray-800">{{ edl.locataire_full_name }}</div>
            </template>
            <div class="text-gray-500 font-medium">Date</div>
            <div class="font-semibold text-gray-800">{{ today }}</div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-gray-200 shadow-sm p-4 sm:p-6 mb-6">
        <div class="flex items-center justify-between mb-4">
            <h2 class="text-lg font-bold text-gray-700">Signer ci-dessous</h2>
            <button @click="clearSignature" type="button"
                class="inline-flex items-center gap-2 text-red-600 hover:text-red-700 border border-red-200 hover:border-red-400 rounded-xl px-4 py-2.5 font-semibold transition active:scale-95 text-sm">
                🗑 Effacer
            </button>
        </div>
    ...
    <!-- Mentions légales -->
    <div class="bg-slate-50 rounded-xl border border-slate-200 px-5 py-4 mb-6 text-sm text-slate-600 leading-relaxed">
        <p>
    ...
    <button @click="submitSignature" :disabled="!hasSigned || submitting"
        class="w-full bg-indigo-600 hover:bg-indigo-700 active:bg-indigo-800 disabled:bg-gray-300 disabled:cursor-not-allowed text-white font-bold py-4 sm:py-5 rounded-2xl shadow-md transition text-lg sm:text-xl tracking-wide">
        {{ submitting ? 'Traitement en cours…' : '✅ Valider et générer le PDF' }}
    </button>
    <p v-if="showEmptyMsg" class="text-center text-red-500 text-sm mt-3">
        Veuillez apposer votre signature avant de valider.
    </p>

    <!-- Loader -->
    <div v-if="submitting" class="mt-8 text-center text-indigo-600">
