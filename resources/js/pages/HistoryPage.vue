...
<div class="fade-in-up">
    ...
    <div class="mb-6 flex flex-wrap items-center justify-between gap-3">
        <div>
            <h1 class="text-xl sm:text-2xl font-extrabold text-gray-900 tracking-tight">Historique des EDL</h1>
            <p class="mt-0.5 text-sm text-gray-500">Tous les états des lieux enregistrés.</p>
        </div>
        <router-link to="/"
            class="inline-flex items-center gap-2 bg-gradient-to-r from-indigo-600 to-violet-600 hover:from-indigo-700 hover:to-violet-700 text-white font-semibold py-2.5 px-4 rounded-xl text-sm shadow-sm shadow-indigo-200 transition min-h-[44px]">
    ...
    <div v-if="loading" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4" role="status" aria-live="polite" aria-label="Chargement…">
        <div v-for="i in 3" :key="i" class="card rounded-2xl overflow-hidden">
            <div class="p-4 border-b border-gray-100">
                ...
                <div class="h-4 w-3/4 bg-gray-200 rounded-lg animate-pulse mb-2"></div>
    ...
    <div v-else-if="edls.length === 0" class="card rounded-2xl px-6 py-16 text-center">
        <div class="inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-gray-100 text-gray-400 mb-4">
            ...
            <div v-for="edl in edls" :key="edl.id"
                ...
                <div class="p-4 border-b border-gray-100/80">
                    <div class="flex items-start justify-between gap-2 mb-2.5">
                        ...
                        <span v-else
                            class="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-semibold shrink-0 bg-amber-50 text-amber-700 ring-1 ring-amber-300/80">
                            En cours
                        </span>
                    </div>
                    <h3 class="text-sm font-bold text-gray-900 leading-snug truncate">{{ edl.adresse }}</h3>
                    <p class="text-xs text-gray-400 mt-0.5">{{ edl.ville }}</p>
                </div>

                <!-- Corps -->
                <div class="px-4 py-3 flex-1 space-y-1.5 text-xs text-gray-600">
                ...
                <!-- Actions -->
                <div class="px-4 py-3 border-t border-gray-100/80 flex gap-2">
                    <template v-if="edl.status === 'en_cours'">
                        <router-link :to="{ name: 'survey', params: { id: edl.id } }"
                            class="flex-1 text-center bg-indigo-600 hover:bg-indigo-700 text-white font-semibold py-2.5 rounded-xl text-sm transition min-h-[44px] flex items-center justify-center gap-1.5 shadow-sm shadow-indigo-200">
                            Reprendre
                    ...
                    <template v-else>
                        <router-link :to="{ name: 'confirmation', params: { id: edl.id } }"
                            class="flex-1 text-center bg-indigo-50 hover:bg-indigo-100 text-indigo-700 font-semibold py-2.5 rounded-xl text-sm transition min-h-[44px] flex items-center justify-center gap-1.5">
    ...
    <Teleport to="body">
        <div v-if="edlToDelete" class="fixed inset-0 z-50 flex items-center justify-center px-4"
            ...
            <!-- Carte -->
            <div class="relative bg-white rounded-2xl shadow-2xl shadow-black/20 max-w-sm w-full p-6 fade-in-up">
                <div class="flex items-start gap-4 mb-5">
                    ...
                    <div>
                        <h2 class="text-base font-bold text-gray-900">Supprimer cet EDL ?</h2>
                        <p class="mt-1 text-sm text-gray-500">
                            <span class="font-semibold text-gray-700">{{ edlToDelete.adresse }}</span>, {{ edlToDelete.ville }}
                        </p>
                        <p class="mt-1 text-xs text-red-500 font-medium">Cette action est irréversible.</p>
                    </div>
                </div>
                <div class="flex gap-3">
                    ...
                    <button @click="confirmDelete" :disabled="deleteLoading"
                        class="flex-1 py-2.5 rounded-xl bg-red-600 hover:bg-red-700 active:bg-red-800 text-white font-bold text-sm transition disabled:opacity-60">
