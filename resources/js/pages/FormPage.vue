...
<div v-else>
    <!-- En-tête compact -->
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm px-4 sm:px-6 py-4 mb-5">
        ...
        <div class="mt-2 flex flex-wrap items-center gap-2">
            ...
            <span v-if="edl.locataire_full_name" class="text-sm font-medium text-gray-700">
                👤 {{ edl.locataire_full_name }}
            </span>
            <span v-if="edl.agent_name" class="text-sm text-gray-500">
                🔧 {{ edl.agent_name }}
            </span>
            <span class="text-sm text-gray-400 ml-auto">📅 {{ today }}</span>
        </div>
    </div>

    <!-- Navigation mobile (< md) : sélecteur d'étape -->
    <div class="md:hidden bg-white rounded-t-xl border border-gray-200 shadow-sm px-3 py-2 flex items-center gap-2">
        <button @click="sidebarOpen = !sidebarOpen"
            class="flex items-center gap-2 text-indigo-700 font-semibold text-sm px-3 py-2 rounded-xl bg-indigo-50 hover:bg-indigo-100 min-h-[44px]"
            ...
            <span class="ml-auto text-xs text-gray-400">{{ currentStep + 1 }}/{{ steps.length }}</span>
    ...
    <div v-show="sidebarOpen" id="mobile-steps"
        class="md:hidden bg-white border-x border-b border-gray-200 shadow-lg px-2 py-2 max-h-72 overflow-y-auto z-40">
        <nav class="space-y-1" role="navigation" aria-label="Étapes du formulaire">
            <button v-for="(step, i) in steps" :key="i"
                ...
                :class="currentStep === i ? 'bg-indigo-600 text-white' : 'text-gray-600 hover:bg-indigo-50 hover:text-indigo-700'"
    ...
    <div class="flex rounded-b-xl md:rounded-xl border border-gray-200 shadow-sm overflow-hidden md:mt-0">
        ...
        <!-- Sidebar desktop -->
        <aside class="hidden md:flex w-56 lg:w-60 shrink-0 bg-white border-r border-gray-200 sticky overflow-y-auto flex-col"
            style="top:56px; max-height:calc(100vh - 56px);" aria-label="Navigation des étapes">
            <!-- Titre sidebar -->
            <div class="px-4 pt-4 pb-2">
                <p class="text-xs font-bold uppercase tracking-widest text-indigo-400">Étapes</p>
            </div>
            <nav class="flex-1 px-2 pb-2 space-y-0.5" role="navigation">
                <button v-for="(step, i) in steps" :key="i"
                    @click="goToStep(i)"
                    class="w-full text-left flex items-center gap-2 px-3 py-2.5 rounded-xl font-medium text-sm transition-all"
                    :class="currentStep === i ? 'bg-indigo-600 text-white shadow-sm' : 'text-gray-600 hover:bg-indigo-50 hover:text-indigo-700'"
                    :aria-current="currentStep === i ? 'step' : null">
        ...
        <!-- Contenu principal -->
        <div class="flex-1 min-w-0 px-4 sm:px-6 py-5 bg-slate-50/60">
            ...
            <div v-for="(step, i) in steps" :key="i" v-show="currentStep === i">
                ...
                <h2 class="text-xl sm:text-2xl font-extrabold text-gray-800 mb-4">{{ step.icon }} {{ step.title }}</h2>

                <!-- TYPE : COMPTEURS -->
                <template v-if="step.type === 'compteurs'">
                    <div class="space-y-5">
                        <div class="bg-white rounded-2xl border border-gray-200 shadow-sm p-5">
                            <h3 class="text-lg font-bold text-gray-700 mb-4">📏 Relevés de compteurs</h3>
                            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                                <div v-for="(label, key) in compteurFields" :key="key">
                                    ...
                                    <input type="number" v-model="formData[key]" step="0.001" min="0"
                                        @input="onFieldChange"
                                        class="w-full border border-gray-300 rounded-xl px-4 py-3 text-base focus:ring-2 focus:ring-indigo-300 focus:border-indigo-400">
                                </div>
                            </div>
                        </div>
                        <div class="bg-white rounded-2xl border border-gray-200 shadow-sm p-5">
                            <h3 class="text-lg font-bold text-gray-700 mb-4">🔑 Clés remises</h3>
                        ...
                        <div class="bg-white rounded-2xl border border-gray-200 shadow-sm p-5">
                            <h3 class="text-base font-bold text-gray-700 mb-3">Photos</h3>
                            <div class="flex gap-2">
                                <button type="button" @click="openPhotoUpload(step.key)"
                                    class="flex-1 flex items-center justify-center gap-2 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 font-semibold rounded-xl border border-indigo-200 py-3 px-4 text-sm transition min-h-[48px]">
                                ...
                                <button type="button" @click="openCameraCapture(step.key)"
                                    class="flex-1 flex items-center justify-center gap-2 bg-slate-50 hover:bg-slate-100 text-slate-700 font-semibold rounded-xl border border-slate-200 py-3 px-4 text-sm transition min-h-[48px]">
                                    <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" stroke-width="1.8" viewBox="0 0 24 24">
                            ...
                            <div v-if="(photos[step.key] || []).length" class="grid grid-cols-3 gap-2 mt-3">
                                <div v-for="photo in (photos[step.key] || [])" :key="photo.id" class="relative group">
                                    <img :src="photo.url" :alt="step.key"
                                        class="rounded-xl object-cover w-full border border-gray-200 shadow-sm" style="height:90px;">
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <template v-else-if="step.type === 'room'">
                    <div class="space-y-4">
                        <div v-for="element in step.elements" :key="element"
                            ...
                            <h3 class="text-lg font-bold text-gray-800 mb-3">{{ element }}</h3>
                            ...
                            <textarea v-model="formData[`${step.key}_${slugify(element)}_obs`]"
                                ...
                                class="w-full border border-gray-300 rounded-xl px-4 py-3 text-base focus:ring-2 focus:ring-indigo-300 focus:border-indigo-400 resize-none">
                            </textarea>
                        </div>
                ...
                <template v-else-if="step.type === 'checklist'">
                    <div class="space-y-4">
                        <div v-for="item in step.items" :key="item"
                            class="bg-white rounded-2xl border border-gray-200 shadow-sm p-5">
                            <h3 class="text-lg font-bold text-gray-800 mb-3">{{ item }}</h3>
                        ...
                        <table class="w-full">
                            <thead>
                                <tr class="bg-indigo-50 border-b border-indigo-100">
                                    <th class="text-left px-4 py-3 text-sm font-bold text-indigo-700">Article</th>
                                    <th class="px-3 py-3 text-sm font-bold text-indigo-700 text-center w-20">Nb</th>
                                    <th v-if="step.withDimension" class="px-3 py-3 text-sm font-bold text-indigo-700 text-center w-28">Dimension</th>
                                    <th class="px-4 py-3 text-sm font-bold text-indigo-700 text-left">Observations</th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr v-for="item in step.items" :key="item"
                                    class="border-b border-gray-100 hover:bg-gray-50">
                                    <td class="px-4 py-3.5 text-base font-medium text-gray-800">{{ item }}</td>
                                    <td class="px-3 py-3.5 text-center">
                                        <input type="number" min="0"
                                            v-model="formData[`${step.key}_${slugify(item)}_nb`]"
                                            @input="onFieldChange"
                                            class="w-16 border border-gray-300 rounded-lg px-2 py-2 text-base text-center focus:ring-2 focus:ring-indigo-300">
                                    </td>
                                    <td v-if="step.withDimension" class="px-3 py-3.5">
                                        <input type="text" placeholder="ex: 140×190"
                                            ...
                                            @input="onFieldChange"
                                            class="w-full border border-gray-300 rounded-lg px-2 py-2 text-sm focus:ring-2 focus:ring-indigo-300">
                                    </td>
                                    <td class="px-4 py-3.5">
                                        <input type="text" placeholder="Observations…"
                                            v-model="formData[`${step.key}_${slugify(item)}_obs`]"
                                            @input="onFieldChange"
                                            class="w-full border border-gray-300 rounded-lg px-3 py-2 text-base focus:ring-2 focus:ring-indigo-300">
                                    </td>
                                </tr>
                            </tbody>
                        </table>
                    ...
                    <div class="space-y-5">
                        <div v-for="section in step.sections" :key="section.groupKey"
                            ...
                            <div class="bg-indigo-50 border-b border-indigo-100 px-5 py-3">
                                <h3 class="text-base font-bold text-indigo-700">{{ section.title }}</h3>
                            </div>
                            <table class="w-full">
                                <thead>
                                    <tr class="border-b border-gray-100 bg-gray-50">
                                        <th class="text-left px-4 py-2.5 text-sm font-semibold text-gray-600">Article</th>
                                        <th class="px-3 py-2.5 text-sm font-semibold text-gray-600 text-center w-20">Nb</th>
                                        <th class="px-4 py-2.5 text-sm font-semibold text-gray-600 text-left">Observations</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <tr v-for="item in section.items" :key="item"
                                        class="border-b border-gray-100 hover:bg-gray-50">
                                        <td class="px-4 py-3 text-base font-medium text-gray-800">{{ item }}</td>
                ...
                <template v-else-if="step.type === 'synthese'">
                    <div class="space-y-4">
                        <div v-for="field in step.fields" :key="field.key"
                            class="bg-white rounded-2xl border border-gray-200 shadow-sm p-5">
                            <p class="text-base font-bold text-gray-800 mb-1">{{ field.label }}</p>
                            <p v-if="field.description" class="text-sm text-gray-500 mb-4">{{ field.description }}</p>
                        ...
                        <div class="bg-white rounded-2xl border border-gray-200 shadow-sm p-5">
                            <label class="block text-base font-bold text-gray-800 mb-2">📝 Observations générales</label>
                ...
                <div class="flex justify-between items-center mt-8 pt-5 border-t border-gray-200">
                    <button @click="goToStep(i - 1)"
                        :class="i === 0 ? 'invisible' : ''"
                        class="flex items-center gap-2 bg-white border-2 border-gray-300 text-gray-700 font-bold px-6 py-3.5 rounded-xl text-base hover:border-indigo-400 hover:text-indigo-700 transition-all">
                        ← Précédent
                    </button>
